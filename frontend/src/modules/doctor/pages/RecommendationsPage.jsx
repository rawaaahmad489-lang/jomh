...
export default function RecommendationsSection({
  isAr,
  mode       = "home",    // "home" | "store"
  storeId    = null,      // مطلوب فقط في mode="store"
  children   = [],        // أطفال الأم (لتخصيص التوصيات)
  myUserId   = null,
  maxCards   = 3,
}) {
...
`;
  ...
  useEffect(() => { fetchAll(); }, [storeId, mode]);
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      if (mode === "store" && storeId) {
        // فقط التوصيات التي تحتوي منتجات من هذا المتجر
        const { data: storeProds } = await supabase
          .from("products")
          .select("product_id")
          .eq("store_id", storeId);

        const prodIds = (storeProds || []).map(p => p.product_id);
        ...
        const ids = [...new Set((recIds || []).map(r => r.recommendation_id))];
      ...
      // ── Personalized for mother (by child age) ────────
      if (mode === "home" && children.length > 0) {
        const ages = children.map(c =>
          Math.floor((Date.now() - new Date(c.birth_date)) / (1000 * 60 * 60 * 24 * 30.44))
        );
      ...
      console.error("RecommendationsSection:", e.message);
    }
    setLoading(false);
  };
  ...
  const addToCart = async (productId) => {
    if (!myUserId) { navigate("/login"); return; }
  ...
  const addAllToCart = async (recProds) => {
    for (const rp of recProds) {
      if (rp.products?.product_id) await addToCart(rp.products.product_id);
    }
    alert(isAr
  ...
  const addComment = async (recId, content) => {
    if (!myUserId || !content.trim()) return false;
    const { error } = await supabase.from("recommendation_comments")
      .insert({ recommendation_id: recId, user_id: myUserId, content: content.trim() });
    if (!error) fetchAll();
    return !error;
  };
  ...
  const commonCardProps = (rec) => ({
    ...
    onAddAllToCart:() => addAllToCart(rec.recommendation_products || []),
  ...
  return (
    <div style={S.wrapper}>
      ...
      {/* ══════════════════════════════════════════════════
          MODE = HOME  →  صفحة الأم
          يعرض: توصيات شخصية + توصيات عامة + زر عرض الكل
      ══════════════════════════════════════════════════ */}
      {mode === "home" && (
        <>
              ...
              <div style={S.blockHeader}>
                <div>
                  <h2 style={S.blockTitle}>
                    ...
                    {isAr ? "توصيات مخصصة لطفلكِ" : "Personalized for Your Baby"}
                  </h2>
                  <p style={S.blockSub}>
                    {isAr
                ...
                <button style={S.viewAllBtn} onClick={() => navigate("/recommendationss")}>
                  {isAr ? "عرض الكل ←" : "View All →"}
                </button>
              </div>
              <div style={S.grid}>
                {personal.map(rec => (
                  <RecCard key={rec.id} {...commonCardProps(rec)} />
                ))}
              </div>
            </div>
          )}
          ...
          {recs.length > 0 && (
            <div style={S.block}>
              ...
              <div style={{ textAlign: "center", marginTop: 24 }}>
                <button style={S.bigBtn} onClick={() => navigate("/recommendationss")}>
          ...
          {personal.length === 0 && recs.length === 0 && (
            <div style={S.empty}>
              <span>💡</span>
      ...
      {/* ══════════════════════════════════════════════════
          MODE = STORE  →  صفحة المتجر
          يعرض: توصيات تحتوي منتجات من هذا المتجر
      ══════════════════════════════════════════════════ */}
      {mode === "store" && (
        <div style={S.block}>
          <div style={S.blockHeader}>
            <div>
              <h2 style={S.blockTitle}>
                ...
                {isAr ? "ماذا يقول أطباؤنا؟" : "What Our Doctors Say"}
              </h2>
              <p style={S.blockSub}>
                {isAr
          ...
          {recs.length === 0 ? (
            <div style={S.empty}>
              <span>💡</span>
              <p>{isAr ? "لا توجد توصيات لهذا المتجر بعد" : "No recommendations for this store yet"}</p>
            </div>
          ) : (
            <>
              <div style={S.grid}>
              ...
              <div style={{ textAlign: "center", marginTop: 20 }}>
                <button style={S.viewAllBtn} onClick={() => navigate("/recommendations")}>
...
const S = {
  wrapper:     { fontFamily: "'Cairo','Poppins',sans-serif" },
  block:       { marginBottom: 36 },
  blockHeader: { display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20, flexWrap: "wrap", gap: 12 },
  blockTitle:  { fontSize: "1.15rem", fontWeight: 900, color: "#333", display: "flex", alignItems: "center", gap: 8, margin: 0 },
  blockSub:    { fontSize: ".82rem", color: "#888", fontWeight: 600, marginTop: 4 },
  dot:         { fontSize: "1.2rem" },
  grid:        { display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 18 },
  viewAllBtn:  { background: "#fdf2f5", color: "#d68b9d", border: "1.5px solid #eab8c6", padding: "9px 18px", borderRadius: 12, cursor: "pointer", fontFamily: "inherit", fontWeight: 800, fontSize: ".82rem", display: "flex", alignItems: "center", gap: 6, whiteSpace: "nowrap", transition: ".3s" },
  bigBtn:      { background: "#d68b9d", color: "#fff", border: "none", padding: "12px 28px", borderRadius: 14, cursor: "pointer", fontFamily: "inherit", fontWeight: 800, fontSize: ".9rem", display: "inline-flex", alignItems: "center", gap: 8, boxShadow: "0 4px 15px rgba(214,139,157,.3)", transition: ".3s" },
  empty:       { textAlign: "center", padding: "32px", color: "#bbb" },
  loadWrap:    { textAlign: "center", padding: "40px" },
  spinner:     { width: 36, height: 36, borderRadius: "50%", border: "4px solid #fdf2f5", borderTopColor: "#d68b9d", animation: "spin .8s linear infinite", margin: "0 auto" },
};
